...
class RemoveDuplication():
    SPECIAL_CHARACTER = '?%@$.,=+-!;/()*"&^:#|\n\t\'[]}{\\0123456789'
    STOPWORDS_PATH = "./tfidf_sim/stopwords.txt"
    ...
    def similarity(self, threshold=0):
        # tokenizer, remove stopword, word count
        item1 = self.tokenizer(self.file1[list(self.file1.keys())[0]])
        listItem = {key:self.tokenizer(self.listFile[key]) for key in list(self.listFile.keys())}
        w = np.array([1.173482  , 0.34364137, 0.7167485 , 0.82080479])
