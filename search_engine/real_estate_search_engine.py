...
class RealEstateSearchEngine:
    def __init__(self, address_weight = 3/5, content_weight = 1/5, type_weight = 1/5, max_result = 100):
        self.address_invert_index : InvertIndex = load(os.path.join(ROOT_DIR,"search_engine","address_invert_index.lib"))
        self.content_invert_index : InvertIndex = load(os.path.join(ROOT_DIR,"search_engine","content_invert_index.lib"))
        self.type_invert_index : InvertIndex = load(os.path.join(ROOT_DIR,"search_engine","type_invert_index.lib"))
    ...
    def find(self, find_str):
        ...
        for doc in list_docs_type:
            if doc in sumup_docs:
                ...
                sumup_docs[doc] = self.type_weight*list_docs_type[doc]
        return sorted(sumup_docs.items(), key=itemgetter(1), reverse=True)[:self.max_result]